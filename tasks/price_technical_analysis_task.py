...
DESCRIPTION_TEMPLATE = (
    ...
    "1. 获取历史K线数据：使用 'Market Data Fetcher' 工具，参数如下：\n"
    "   - data_type: 'futures_history'\n"
    "   - asset_class: 'future'\n"
    "   - region: '{region}'\n"
    "   - ticker: '{ticker}'\n"
    ...
    "2. 获取相关新闻资讯：再次使用 'Market Data Fetcher'，参数如下：\n"
    "   - data_type: 'news'\n"
    "   - asset_class: 'future'\n"
    "   - region: '{region}'\n"
    "   - ticker: '{ticker}'\n"
    "   注意：工具将自动构建 full_symbol 并搜索最新消息\n\n"

    "3. 撰写分析报告：将以上所有信息整合，并严格遵循给定的 Markdown 模板格式，撰写最终的技术分析报告。\n"
...
def create_price_technical_analysis_task(
    ...
    final_expected_output = (report_template or PRICE_TECHNICAL_REPORT_TEMPLATE).format(
        commodity_name=commodity_name,
        start_date=start_date,
        end_date=end_date
    )
